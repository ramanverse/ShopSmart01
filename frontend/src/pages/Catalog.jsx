...
const Catalog = ({ setCartCount }) => {
  ...
  const toggleWishlist = (id) => {
    ...
    if (updated.includes(id)) {
      ...
      toast.info("Removed from your selection.", { theme: "dark", autoClose: 1000 });
    } else {
      updated.push(id);
      toast.success("Added to your selection.", { theme: "dark", icon: "✨", autoClose: 1000 });
    }
    setWishlist(updated);
    localStorage.setItem('ethereal_wishlist', JSON.stringify(updated));
  };

  const categories = ["Men", "Women", "Accessories"]; // Removed Kids
  const subCategories = ["Topwear", "Bottomwear", "Outerwear", "Dresses", "Footwear", "Accessories"];
    ...
    api.get(`/products?${queryParams.toString()}`)
      .then(res => {
        if(res.data.success) {
          ...
          // Robust check: if we have fewer products than the total available, set hasMore to true
          const currentTotal = filters.page === 1 ? newProducts.length : products.length + newProducts.length;
    ...
    <div className="bg-cloud-dancer min-h-screen pt-32 pb-20 px-8 lg:px-20">
      <div className="max-w-screen-2xl mx-auto">
        <header className="mb-20">
          ...
          >
            The Collection <span className="italic font-normal text-stone-gray">— {filters.category}</span>
          </motion.h1>
          ...
          <main className="flex-1">
            <div className="flex justify-between items-center mb-12">
                ...
                <div className="flex items-center gap-4">
                    <span className="font-sans text-[10px] uppercase font-bold tracking-[0.3em] text-stone-gray">Sort By</span>
                    <select 
                        value={filters.sort} 
                  ...
                  >
                    <div className="relative aspect-[3/4] overflow-hidden bg-stone-gray/10 group-hover:shadow-[0_20px_50px_rgba(212,175,55,0.15),_inset_0_0_15px_rgba(255,255,255,0.05)] transition-all duration-700">
                      <Link to={`/product/${prod._id}`}>
                        ...
                        <div className="absolute inset-0 bg-luxury-black/40 opacity-0 group-hover:opacity-100 transition-opacity duration-500 flex flex-col justify-end p-8">
                            ...
                            <button 
                                onClick={(e) => { e.preventDefault(); handleAddToCart(prod._id); }}
                      ...
                      <button 
                        onClick={(e) => { e.preventDefault(); toggleWishlist(prod._id); }}
                      ...
                      >
                         <FiHeart fill={wishlist.includes(prod._id) ? "currentColor" : "none"} className="text-sm" />
                      </button>
                    ...
                    <div className="mt-10 flex justify-between items-baseline">
                      <div className="space-y-2">
                        ...
                        <p className="font-sans text-[10px] uppercase font-bold tracking-[0.3em] text-stone-gray/60">{prod.subCategory}</p>
                      </div>
                      <p className="font-sans font-bold text-base tracking-widest">${prod.basePrice}</p>
                    </div>
                  </motion.div>
                ))}
              </AnimatePresence>
              ...
              <div className="mt-32 flex justify-center">
                ...
                >
                  <div className="w-16 h-16 rounded-full border border-luxury-black/20 flex items-center justify-center group-hover:border-luxury-black transition-colors duration-500">
                    <FiPlus className={`text-2xl transition-transform duration-500 group-hover:rotate-90 ${loading ? 'animate-spin' : ''}`} />
                  </div>
            ...
            {!loading && products.length === 0 && (
               ...
               >
                 <h2 className="font-serif text-3xl italic text-stone-gray">Nothing meets the criteria.</h2>
                 <p className="font-sans text-xs uppercase tracking-widest mt-4">Try adjusting your filters or search.</p>
               </motion.div>
            )}
          </main>
        </div>
